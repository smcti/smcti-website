...
const Footer = () => {
  ...
  return (
    <footer className="mt-auto bg-cello-800 text-zircon">
        <div className='mt-32 section-default flex flex-col gap-16
        md:flex-row'>
            ...
            {Object.values(footerData).map((item, index) => {
              ...
              return <div key={index} className='flex flex-col gap-4'>
                ...
                {Object.values(item).map((subItem, subIndex) => {
                  return (
                    <a href={subItem.path} key={subIndex}>{subItem.name}</a>
                  )
                })}
              </div>
            })}
        ...
        <div className='flex flex-col text-center section-default mb-32 gap-16 mt-8'>
          <hr />
          <p>Todos os direitos reservados, &copy; 2023 Secretaria de Ciência, Tecnologia e Inovação de Pato Branco - PR</p>
        </div>
    </footer>
  )
}
