              ...
              {item.dropdown ? (
                <Menu>
                  <div className='relative'>
                    ...
                    <Menu.Items
                      className={
                        "flex flex-col absolute w-fit shadow overflow-hidden mt-4 min-w-full rounded-lg"
                      }
                    >
                      {item.dropdown.map(
                          ...
                          <Menu.Item key={dropdownIndex}>
                            {({ active }: { active: any }) => (
                              <a
                                ...
                                className='px-4 py-2 text-gray-900 bg-white hover:bg-gray-100  whitespace-nowrap'
                              >
                                {dropdownItem.name}
                              </a>
                            )}
                ...
                {item.dropdown ? (
                  <Menu>
                    <div className=''>
                      ...
                      <Menu.Items
                        className={
                          "flex flex-col bg-white w-full justify-end"
                        }
                      >
                        {item.dropdown.map(
                          (dropdownItem, dropdownIndex) => (
                            <Menu.Item key={dropdownIndex}>
                              {({ active }: { active: any }) => (
                                <a
                                  href={dropdownItem.path}
                                  className='py-2 hover:text-gray-300 text-right '
                                >
                                  {dropdownItem.name}
                                </a>
                              )}
