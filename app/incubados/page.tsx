...
export default function IncubadosSection() {
  ...
  // 1. Ordenação Alfabética utilizando useMemo para performance
  const sortedIncubadas = useMemo(() => {
    return [...incubadasData].sort((a, b) =>
      a.name.localeCompare(b.name)
    );
  }, []);

  const carouselList = sortedIncubadas.map(item => ({
    name: item.name,
    logo: item.logo
  }));
  ...
  return (
    <section className="py-16 overflow-hidden relative min-h-screen z-0">
      ...
      <div className="w-full mt-20 pt-16 border-t border-gray-100 relative">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-yellow-400 to-amber-500 text-white px-6 py-2 rounded-full text-sm font-bold shadow-lg flex items-center gap-2">
          <Trophy size={16} /> Empresas Graduadas
        </div>
